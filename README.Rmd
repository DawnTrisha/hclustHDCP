...
Now we note that we need to consider the following points while using the functions,
 ...
 - It is sufficient for the user to enter either the data matrix (X) or the distance matrix (D).  
...
## detect_multiple_cp : 
Detects multiple change-points in a data sequence arranged chronologically. Here also we need to consider the same three points that we mentioned earlier. Additionally we also need to supply the number of change-points to detect (1 < numcp < n). Here is an illustration.
